 ...
 * are:
 * <ol>
 * <li>Standard Enthalpy
 * <li>Standard Entropy
 * <li>List of Heat Capacities at the temperatures specified by the type
 * <ol>
 *
 * @author blurock
 */
public class BensonThermodynamicBase extends ChemObject implements ThermodynamicInformation, Comparable<BensonThermodynamicBase> {
	...
	public void addInto(BensonThermodynamicBase toAdd) throws ThermodynamicComputeException {
		...
		while (pnew.hasNext()) {
			...
			HeatCapacityTemperaturePair pairthis = findTemperatureMatch(pairnew);
			pairthis.addInto(pairnew);
		}
	}

	HeatCapacityTemperaturePair findTemperatureMatch(HeatCapacityTemperaturePair pair) {
		HeatCapacityTemperaturePair ans = null;
		...
		while (ans == null) {
			...
			if (temperatureCloseEnough(pairthis.getTemperatureValue(), pair.getTemperatureValue())) {
				ans = pairthis;
			}
		}
		return ans;
	...
	boolean temperatureCloseEnough(double t1, double t2) {
		boolean ans = false;
	...
	public BensonThermodynamicBase reformulateWithTemperatureSet(double[] temperatures)
	 ...
	 * capacities
	 *
	 * Using a linear extrapolation of the heat capacities between the values given,
	 * the integral delta H = Hstd + integral( Cp(T)dt )
	 *
	 * Between each pair of heat capacities, cplower and cpupper, integral( Cp(T)dt
	 * ) from lower T to upper T is (upperT - lowerT)*(cplower+cpupper)/2
	 *
	 * The estimate is only valid above the temperature of the lowest heat capacity.
	 * The heat capacity above that of the highest is assumed to be constant.
	 *
	 * @param temperature
	...
	public double computeEnthalpy(double temperature) throws ThermodynamicComputeException {
		...
		// ThermoInfoType type =
		// ThermodynamicProperties.getTherodyInfoType(thermodynamicType);
		...
		if (t < 0) {
			throw new ThermodynamicComputeException(
					"Error in computing Enthalpy: Temperature out of range " + temperature + "<" + tlower);
		}
		double factor = 0.0;
		if (index + 1 >= setOfHeatCapacities.size()) {
			factor = cplower * t;
		} else {
			int setsize = setOfHeatCapacities.size();
			HeatCapacityTemperaturePair[] arraySet = new HeatCapacityTemperaturePair[setsize];
