...
public class GetSubstructureMatches {
	 ...
	 * @throws org.openscience.cdk.exception.CDKException
	 */
	public List<List<RMap>> getBondMatches(IAtomContainer mol1, QueryMoleculeWithMetaAtoms substructure)
			throws CDKException {
	 ...
	 * @throws org.openscience.cdk.exception.CDKException
	 */
	public List<List<RMap>> getAtomMatches(IAtomContainer mol1, QueryMoleculeWithMetaAtoms substructure)
			throws CDKException {
		QueryMoleculeWithMetaAtoms qmol1 = new QueryMoleculeWithMetaAtoms(mol1);
	 ...
	 * @throws org.openscience.cdk.exception.CDKException
	 */
	public Integer[] getMatchesForAtomWithBondList(IAtomContainer mol1, IAtomContainer mol2, int atmnum,
			List<List<RMap>> matches) throws CDKException {
	 ...
	 * @return
	 */
	public ArrayList<ArrayList<IBond>> isolateListOfBondsFromMatches(IAtomContainer mol1, IAtomContainer mol2,
			List<RMap> matches) {
	...
	List<List<RMap>> singleBondSubstructureCase(QueryMoleculeWithMetaAtoms qmol1, QueryMoleculeWithMetaAtoms qmol2) {
		...
		if (debug) {
			System.out.println("singleBondSubstructureCase: \n");
		...
		while (iter.hasNext()) {
			...
			if (debug) {
				System.out.println("Bond atom 1\n" + molatm1.toString());
				System.out.println("Bond atom 2\n" + molatm2.toString());

				System.out.println("11: " + molatm1.matches(atm1) + "  22: " + molatm2.matches(atm2));
				System.out.println("12: " + molatm1.matches(atm2) + "  21: " + molatm2.matches(atm1));
			}

			if (molatm1.matches(atm1) && molatm2.matches(atm2)) {
				if (debug) {
					System.out.println("Atoms 11  22 matches");
				}
				...
				match.add(match2);
				map.add(match);
			} else if (molatm1.matches(atm2) && molatm2.matches(atm1)) {
				if (debug) {
					System.out.println("Atoms 12  21 matches");
				}
