 ...
 *
 * The atomic structure is represented by 20 I3 (three character integer) fields
 * with the number of the corresponding atom. The atoms, in order are:
...
public class JThergasAtomicStructure {
	...
	/**
	 * The constructor this sets up the 20 atomic numbers of the elements in the
	 * line
	 * 
	 */
	...
	public void parse(String line) throws JThergasReadException {
		...
		try {
			...
			throw new JThergasReadException("Error in Parsing Atomic Structure Line: table number\n" + line);
		}
		try {
			groupNumber = Integer.parseInt(groupNumberS);

		} catch (NumberFormatException ex) {
			throw new JThergasReadException("Error in Parsing Atomic Structure Line: group number\n" + line);
		}
		try {
			numberOfAtoms = Integer.parseInt(numberOfAtomsS);

		} catch (NumberFormatException ex) {
			throw new JThergasReadException("Error in Parsing Atomic Structure Line: number of atoms\n" + line);
		}
		...
		for (int i = 0; i < 20; i++) {
			if (atomsS[i].length() == 0) {
				...
				try {
					...
					throw new JThergasReadException(
							"Error in Parsing Atomic Structure Line: atom  " + i + " '" + atomsS[i] + "'");
				}
